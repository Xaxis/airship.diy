...
export const BASELINE_ARRANGEMENT: Configuration = {
  ...
  compartments: [
    ...
    },
    {
      ...
      shell: false,
      enclosed: false,
      note: 'THE ANSWER TO THE ONE GATE THAT WAS FAILING. Twenty kelvin of solar superheat moves this envelope\'s lift by more than two tonnes against a landing trim of six hundred kilograms, so a vehicle sitting on the water at dawn takes itself off by mid-afternoon and one trimmed for the afternoon presses two tonnes onto its gear before dawn. No passive device can be sized for a load that swings by that factor twice a day. The ocean can: 2.2 m3 of bladder and a pump that moves 74 kg a minute on 121 W, sized for a clearing overcast rather than for the day because the envelope\'s thermal time constant is tens of minutes. Empty in flight, because there is nothing to pump from up there.',
    },
    {
      id: 'exercise',
  ...
  propulsors: [
    {
      ...
      mass: 190,
      note: 'On an outrigger at the widest station, near the centre of mass, so vectored thrust translates the ship instead of pitching it. ALL FOUR UNITS ARE IDENTICAL, and that is a propulsor-out result rather than a procurement convenience: with two 6 m units and two 5 m ones, losing a big one removed 30 percent of the static thrust rather than 25, which took the worst-case single failure below the landing trim the trim was supposedly set by. A fleet of identical units is also the only fleet worth carrying spares for a year from the nearest chandlery.',
    },
    {
      id: 'starboard-mid',
      station: 0.45,
